class Enemy extends Entity {
    ...
    update() {
        ...
        let isPlayerInSpotRadius = distanceToPlayer < this.minSpotRadius +
            this.spotRadius * this.difficulty && distanceToPlayer > 0;
        let isUserAwayAndWasNotCaught = !(isPlayerInSpotRadius || this.playerWasCaught);

        if(isUserAwayAndWasNotCaught) {
            this.speed = 0;
    ...
    testUserVisibility() {
        if(this.canTestFire) {
            let sampleBullet = this.createTestBullet();
            getGameManager().entities.push(sampleBullet);
            this.canTestFire = false;
            this.setTestDelay(sampleBullet);
        }
    }

    setTestDelay(sampleBullet) {
        setTimeout(() => {
    ...
    fire() {
        if(this.canFire) {
            ...
            this.canFire = false;
            this.setShootingDelay(bullet);
        }

    }

    setShootingDelay(bullet) {
        setTimeout(() => {
            let entity = getGameManager().entity(this.name);
